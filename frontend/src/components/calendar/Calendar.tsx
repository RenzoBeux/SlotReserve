...
const Calendar = ({
  ...
  const calendarDays = useMemo(() => {
    ...
    const daysToDisplay =
      getDay(monthStart) + getDaysInMonth(monthStart) + (6 - getDay(monthEnd));
  ...
  const getSlotAvailabilityInfo = (daySlots: TimeSlotType[]) => {
    const flexibleSlots = daySlots.filter(
      (slot) => slot.bookingMode === "FLEXIBLE"
    );
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isToday(day) &&
                  "border border-bookify-300 dark:border-bookify-700",
