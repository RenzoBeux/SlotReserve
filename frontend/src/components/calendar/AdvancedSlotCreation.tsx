...
import { format, addDays, getDay, setHours, setMinutes, addMinutes } from 'date-fns';
...
export const AdvancedSlotCreation = ({ onSubmit, onCancel }: AdvancedSlotCreationProps) => {
    ...
    while (currentDate <= endDate) {
      ...
      if (selectedDays.includes(weekday.toString())) {
        ...
        if (bookingMode === "FLEXIBLE") {
          ...
          // For FIXED mode, create multiple slots based on duration and breaks
          const [startHour, startMinute] = startTime.split(':').map(Number);
          const [endHour, endMinute] = endTime.split(':').map(Number);
          
          let slotStart = setHours(setMinutes(new Date(currentDate), startMinute), startHour);
          const dayEnd = setHours(setMinutes(new Date(currentDate), endMinute), endHour);
          ...
          while (slotStart < dayEnd) {
            ...
            slotStart = addMinutes(slotStart, parseInt(slotDuration, 10) + parseInt(breakDuration, 10));
          }
        }
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <RadioGroup 
          value={bookingMode} 
          onValueChange={(value) => setBookingMode(value as "FIXED" | "FLEXIBLE")}
      ...
      {bookingMode === "FLEXIBLE" && (
        <Alert>
          <AlertDescription>
            Flexible slots will be created as full-day availability for each selected day, allowing clients to choose their own times within your set hours.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="pt-2">
        <div className="bg-muted p-3 rounded-md text-sm">
          ...
          <p>
            Creating {selectedDays.length > 0 ? 
              WEEKDAYS.filter(day => selectedDays.includes(day.value))
                .map(day => day.label)
                .join(', ') : 'no days'} 
            {' '}from {startDate ? format(startDate, 'MMM d, yyyy') : '–'} to {endDate ? format(endDate, 'MMM d, yyyy') : '–'}
          </p>
          {bookingMode === "FIXED" ? (
            <p>
              {slotDuration} minute slots with {breakDuration} minute breaks between {startTime} and {endTime}
            </p>
          ) : (
            <p>
          ...
          <p>
            {bookingMode === "FIXED" ? "Fixed" : "Flexible"} booking mode with {maxBookings} max booking{maxBookings > 1 ? 's' : ''} per slot
          </p>
        </div>
      </div>
